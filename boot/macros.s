...
.macro mInitSegments
  ...
  mov  ax, cs            # CS is set to 0x0, because that is where boot sector is loaded (0:7c00)
  mov  ds, ax            # DS = CS = 0x0 
  mov  es, ax            # ES = CS = 0x0
  mov  ss, ax            # SS = CS = 0x0
...
.macro mReadFile loadsegment, fatsegment
  ...
  read_file_next_sector:
    ...
    test    cx, 1                     # See which way to shift
...
.endm
 ...
 # get bootsector data that was originally loaded by bootstrapper
 # from memory occupied by bootstrapper
...
# 
# Set the PE-bit in the CR0 register to
...
# 
# Let all data segments point to global
# descriptor 2 (the data segment descriptor).
# Let the stack grow downwards from 0x30000.
# 
